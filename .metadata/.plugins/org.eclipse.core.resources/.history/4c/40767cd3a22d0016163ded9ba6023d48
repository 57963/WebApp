...
public class BookServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		case "rem":
			...
			break;
		case "add":
			addtoSession(req.getParameter("addName"+req.getParameter("sess")),Integer.parseInt(req.getParameter("sess")));
			break;
		}
		res.sendRedirect("/book?date=" + date);
	}
	
	private void addtoSession(String name, int sess) {
		int childID = 0;
		...
		try {
			session.absolute(1);
			for(int i = 0; i<10;i++){
				if(session.getInt(i+5)!=0){
					space = i+2;
				}
			}
			if(space<9){
				colName="booked" + space;
			} else {
				colName="reserve"+ (space-8);
			}
			sql.doUpdate("UPDATE timeslots SET "+colName+" = '"+childID+"' WHERE ID = '"+sess+"'");
		} catch (SQLException e) {
			e.printStackTrace();
		}
		sql.close();
